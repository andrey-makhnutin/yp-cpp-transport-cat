...
void Snowman::Draw(svg::ObjectContainer &obj_container) const {
  ...
  obj_container.Add(
      svg::Circle().SetCenter( { center_.x, center_.y + 5 * head_radius_ })
          .SetRadius(2 * head_radius_).SetFillColor(SNOWMAN_FILL_COLOR)
          .SetStrokeColor(SNOWMAN_STROKE_COLOR));
  obj_container.Add(
      svg::Circle().SetCenter( { center_.x, center_.y + 2 * head_radius_ })
          .SetRadius(1.5 * head_radius_).SetFillColor(SNOWMAN_FILL_COLOR)
          .SetStrokeColor(SNOWMAN_STROKE_COLOR));
  obj_container.Add(
      svg::Circle().SetCenter( { center_.x, center_.y }).SetRadius(head_radius_)
          .SetFillColor(SNOWMAN_FILL_COLOR).SetStrokeColor(
          SNOWMAN_STROKE_COLOR));
}
