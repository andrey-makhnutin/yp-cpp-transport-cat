...
void TestAddStop() {
  ...
  // запрещаем дважды добавлять одну и ту же остановку (пока не требуется уметь
  // обновлять данные)
...
void TestAddBus() {
  ...
  ASSERT_DOESNT_THROW(
      tc.AddBus("751"s, RouteType::LINEAR,
                {"Rasskazovka"s, "Marushkino"s, "Marushkino"s}));

  // в кольцевых маршрутах первая и последняя остановки должны совпадать
  ASSERT_THROWS(
      tc.AddBus("752"s, RouteType::CIRCULAR, {"Rasskazovka"s, "Marushkino"s}),
      invalid_argument);
  ASSERT_DOESNT_THROW(
      tc.AddBus("752"s, RouteType::CIRCULAR,
                {"Rasskazovka"s, "Marushkino"s, "Rasskazovka"s}));

  // запрещаем дважды добавлять один и тот же маршрут (пока не требуется уметь
  // обновлять данные)
  ASSERT_THROWS(
      tc.AddBus("750"s, RouteType::LINEAR, {"Rasskazovka"s, "Marushkino"s}),
      invalid_argument);
...
void TestGetBusStats() {
  {
    ...
    tc.AddStop("Rasskazovka"s, {55.632761, 37.333324});
    tc.AddStop("Marushkino"s,
               {55.632761, 37.3492554327});  // километр на восток
    tc.AddStop("Tolstopaltsevo"s,
               {55.6417542160555, 37.3492554327});  // километр на юг
    tc.AddStop("Biryulyovo Zapadnoye"s,
               {55.632761, 37.3492554327});  // километр на запад
    tc.SetDistance("Marushkino"sv, "Tolstopaltsevo"sv,
                   1001);  // + 1 (есть расстояние в ту сторону)
    tc.SetDistance("Tolstopaltsevo"sv, "Marushkino"sv,
                   1004);  // + 4 (есть расстояние в ту сторону)
    tc.SetDistance(
        "Tolstopaltsevo"sv, "Biryulyovo Zapadnoye"sv,
        1016);  // + 16 + 16 (есть расстояние в ту или обратную сторону)
    tc.AddBus("750"s, RouteType::LINEAR,
              {"Rasskazovka"s, "Marushkino"s, "Tolstopaltsevo"s,
               "Biryulyovo Zapadnoye"s});
    auto bi = tc.GetBusStats("750"sv);
    ASSERT(bi.has_value());
    // линейный маршрут A -> B -> C -> D -> C -> B -> A: 7 остановок, 4
    // уникальные, 6 км
...

  {
    ...
    tc.SetDistance("Marushkino"sv, "Tolstopaltsevo"sv,
                   1001);  // + 1 (есть расстояние в ту сторону)
    tc.SetDistance("Tolstopaltsevo"sv, "Marushkino"sv,
                   1004);  // + 0 (не должно быть учтено)
    tc.SetDistance("Biryulyovo Zapadnoye"sv, "Tolstopaltsevo"sv,
                   1016);  // + 16 (есть расстояние в обратную сторону)
