...
#define ASSERT_NOT_INT(_node) \
  ASSERT(!_node.IsInt());     \
  ASSERT_THROWS(_node.AsInt(), logic_error);
#define ASSERT_NOT_DOUBLE(_node) \
  ASSERT(!_node.IsDouble());     \
  ASSERT_THROWS(_node.AsDouble(), logic_error);
#define ASSERT_NOT_PURE_DOUBLE(_node) ASSERT(!_node.IsPureDouble());
#define ASSERT_NOT_BOOL(_node) \
  ASSERT(!_node.IsBool());     \
  ASSERT_THROWS(_node.AsBool(), logic_error);
#define ASSERT_NOT_STRING(_node) \
  ASSERT(!_node.IsString());     \
  ASSERT_THROWS(_node.AsString(), logic_error);
#define ASSERT_NOT_NULL(_node) ASSERT(!_node.IsNull());
#define ASSERT_NOT_ARRAY(_node) \
  ASSERT(!_node.IsArray());     \
  ASSERT_THROWS(_node.AsArray(), logic_error);
#define ASSERT_NOT_DICT(_node) \
  ASSERT(!_node.IsMap());      \
  ASSERT_THROWS(_node.AsMap(), logic_error);
...
void TestIntNode() {
  ...
  {
    string json_str =
        to_string(static_cast<double>(numeric_limits<int>::max()) + 1);
    StripDot(json_str);
...
void TestArrayNode() {
  ...
  ASSERT_EQUAL(ParseJSON("[ \t\r\n 1 \t\r\n , \t\r\n 2 \t\r\n ]"s).GetRoot(),
               (Node{Array{1, 2}}));
  ASSERT_EQUAL(ParseJSON("[true,false]"s).GetRoot(),
  ...
  ASSERT_EQUAL(ParseJSON("[true,null]"s).GetRoot(),
               (Node{Array{true, nullptr}}));
  ASSERT_EQUAL(ParseJSON("[\"hello\"]"s).GetRoot(), Node{Array{"hello"s}});
  ASSERT_EQUAL(ParseJSON("[\"hello\", \"world\"]"s).GetRoot(),
               (Node{Array{"hello"s, "world"s}}));
  ...
  Node complex_node{Array{1, 2.3, true, "hello"s, nullptr, Array{1, 2, 3},
                          Dict{{"foo"s, "bar"s}}}};
...
void TestDictNode() {
  ...
  ASSERT_EQUAL(ParseJSON("{ }"s).GetRoot(), Node{Dict{}});
  ASSERT_EQUAL(ParseJSON("{ \t\r\n }"s).GetRoot(), Node{Dict{}});
  ASSERT_EQUAL(ParseJSON("{\"hello\":1}"s).GetRoot(),
               (Node{Dict{{"hello"s, 1}}}));
  ASSERT_EQUAL(ParseJSON("{\"hello\" : 1}"s).GetRoot(),
               (Node{Dict{{"hello"s, 1}}}));
  ASSERT_EQUAL(
      ParseJSON("{ \t\r\n \"hello\" \t\r\n : \t\r\n 1 \t\r\n }"s).GetRoot(),
      (Node{Dict{{"hello"s, 1}}}));
  ASSERT_EQUAL(ParseJSON("{\"hello\":1,\"world\":2}"s).GetRoot(),
               (Node{Dict{{"hello"s, 1}, {"world"s, 2}}}));
  ASSERT_EQUAL(ParseJSON("{\"hello\":-1}"s).GetRoot(),
               (Node{Dict{{"hello"s, -1}}}));
  ASSERT_EQUAL(ParseJSON("{\"hello\":+1}"s).GetRoot(),
               (Node{Dict{{"hello"s, 1}}}));
  ASSERT_EQUAL(ParseJSON("{\"hello\":.1}"s).GetRoot(),
               (Node{Dict{{"hello"s, 0.1}}}));
  ASSERT_EQUAL(ParseJSON("{\"hello\":1., \"world\":2}"s).GetRoot(),
               (Node{Dict{{"hello"s, 1.0}, {"world"s, 2}}}));

  string complex_node_str =
      "{\"key1\":1,\"key2\":2.3,\"key3\":true,\"key4\":\"hello\",\"key5\":null,\"key6\":[1,2,3],\"key7\":{\"foo\":\"bar\"}}"s;
  Node complex_node{Dict{{"key1"s, 1},
                         {"key2"s, 2.3},
                         {"key3"s, true},
                         {"key4"s, "hello"s},
                         {"key5"s, nullptr},
                         {"key6"s, Array{1, 2, 3}},
                         {"key7"s, Dict{{"foo"s, "bar"s}}}}};
  ASSERT_EQUAL(ParseJSON(complex_node_str).GetRoot(), complex_node);
  ...
  ASSERT(ParseJSON(R"({"hello":1,"world":2})"s).GetRoot() ==
